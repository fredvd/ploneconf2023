...
STACK_NAME=ploneconf2023-fredvd-tangrama-com-br
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t fredvd/ploneconf2023-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t fredvd/ploneconf2023-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name ploneconf2023-backend-acceptance -d fredvd/ploneconf2023-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name ploneconf2023-frontend-acceptance --link ploneconf2023-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d fredvd/ploneconf2023-frontend:acceptance
